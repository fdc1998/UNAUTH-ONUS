...
def get_onu_by_serial(session, serial):
    try:
        cmd = """show whitelist phy-sn""".format(serial=serial)
        session.write(("cd onu" + '\r').encode('latin-1'))
        index, match_obj, text = session.expect(["onu".encode('latin-1')], timeout=3)
        if match_obj:
...
def deauth_onu(session, serial):
    try:
        cmd = f'set whitelist phy_addr address {serial} password null action delete'
        session.write(("cd onu" + '\r').encode('latin-1'))
        index, match_obj, text = session.expect(["onu".encode('latin-1')], timeout=3)
        if match_obj:
            session.write(bytes(cmd + '\n', 'utf-8'))
